...
const userPasswordLookup = `-- name: UserPasswordLookup :one
SELECT id, created_at, updated_at, email, hashed_password, is_chirpy_red FROM users
WHERE email = $1
`

func (q *Queries) UserPasswordLookup(ctx context.Context, email string) (User, error) {
	row := q.db.QueryRowContext(ctx, userPasswordLookup, email)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
