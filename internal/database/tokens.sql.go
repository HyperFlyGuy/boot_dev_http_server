...
const tokenRevokeUpdate = `-- name: TokenRevokeUpdate :exec
UPDATE refresh_tokens
SET updated_at = NOW(), revoked_at = NOW()
WHERE token = $1
`

func (q *Queries) TokenRevokeUpdate(ctx context.Context, token string) error {
	_, err := q.db.ExecContext(ctx, tokenRevokeUpdate, token)
	return err
}
